...
PYTHONPATH="$ROOT" python3 -m alp.cli bundle run --bundle "$ROOT/samples/add.alppb" --key "$ROOT/samples/dev_hmac.key" >/tmp/openalp-bundle-run-bin.out
